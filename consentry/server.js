...
// Session — used only for the admin panel.
// SESSION_SECRET must be set in production (Render env var).
// ADMIN_PASSWORD must also be set; if it's unset we refuse all logins (fail closed).
app.use(
  session({
    ...
    cookie: {
      httpOnly: true,
      sameSite: 'lax',
      // Mark Secure when running behind the Render HTTPS proxy.
...
// Simple per-IP brute-force throttle for the login endpoint.
const loginAttempts = new Map();
setInterval(() => loginAttempts.clear(), 60_000).unref();

app.post('/api/admin/login', (req, res) => {
  // Fail closed: if ADMIN_PASSWORD is not set, nobody can log in.
  ...
  loginAttempts.set(ip, 0); // reset on success
  res.json({ ok: true });
});
...
// Bulk scan: accepts up to 25 URLs, runs 4 concurrently, persists results.
